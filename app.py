...
#rom torchao.quantization import quantize_, int8_weight_only
import gc
...
def generate(image_input, audio_input, pose_input, width, height, length, steps, sample_rate, cfg, fps, context_frames, context_overlap, quantization_input, seed):
    global vae, reference_unet, denoising_unet, pose_net, audio_processo
    gc.collect()
    ...
    for index in range(start_idx, start_idx + length):
        ...
        im = draw_pose_select_v2(detected_pose, imh_new, imw_new, ref_w=width)
        im = np.transpose(np.array(im),(1,2,0))

        # force resize to target canvas
        im = np.array(Image.fromarray(im).resize((width, height)))
            ...
            with gr.Column():
                ...
                with gr.Group():
                    ...
                    with gr.Row():
                        ...
                        context_frames = gr.Number(label="上下文框架（默认12）", value=8)
                        context_overlap = gr.Number(label="上下文重叠（默认3）", value=2)
                    with gr.Row():
